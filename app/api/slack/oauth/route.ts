...
export async function GET(request: NextRequest) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.redirect(`${appUrl}/?slack=error`)
        }
        ...
        if (returnTo === 'integrations') {
            return NextResponse.redirect(`${appUrl}/integrations?setup=slack`)
        } else {
            return NextResponse.redirect(`${appUrl}/?slack=installed`)
        }
    } catch (error) {
        console.error('slack oauth error', error)
